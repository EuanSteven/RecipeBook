...
def postprocess(output_filename, options):
    ...
    if not options.quiet:
        print('[OK] NOTESHRINK - Running "{}"...'.format(cmd), end=' ')
        sys.stdout.flush()
...
palette, as well as a mask corresponding to the foreground pixels.
    ...
    if not options.quiet:
        print('[OK] NOTESHRINK - Getting Palette')
...
the palette.
    ...
    if not options.quiet:
        print('[OK] NOTESHIRNK - Applying Palette')
...
the background color to pure white.
    ...
    if not options.quiet:
        print('[OK] NOTESHRINK - Saving {}...'.format(output_filename))
    ...
    #output_img = output_img.rotate(90)
...
def notescan_main(options):
    ...
    for input_filename in filenames:
        ...
        if not options.quiet:
            print('[OK] NOTESHRINK - Opened', input_filename)
